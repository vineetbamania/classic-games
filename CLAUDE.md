...
Content sources are all free/open and serve **https** (an https page can't play http audio — mixed content). **Search merges four song sources** (see `searchAll` in app.js); each runs in parallel with its own failure path + a 10s guard so a slow/dead source can't hang the search. Results are tagged by `src` and ordered full-songs-first (saavn → audius → yt → itunes):
- `js/jiosaavn.js` — **JioSaavn = full mainstream songs (Bollywood + English)**, the primary source. JioSaavn's own API has no CORS and DES-encrypts media URLs, so this hits a JioSaavnAPI **backend** that decrypts + adds CORS. `JIOSAAVN_HOSTS` is a **list of instances tried in order** (resilience — public demos rate-limit/die) and the parser understands **both** response shapes: `flat` ([cyberboysumanjay/JioSaavnAPI](https://github.com/cyberboysumanjay/JioSaavnAPI), array w/ `media_url`) and `saavndev` ([sumitkolhe/jiosaavn-api](https://github.com/sumitkolhe/jiosaavn-api), `data.results[].downloadUrl[]`). Forces https, decodes HTML entities, picks the top quality. For reliability deploy your own (Vercel/Render, India region) and put it first in the list. Search-only.

**Rejected song sources (don't re-attempt — evaluated and ruled out):** *Gaana* (every public instance is dead or HTTP 451 "blocked for legal reasons"); *MusicBrainz* (metadata DB — **no audio/stream URLs at all**, only useful for enrichment); *Openwhyd* (tracks are `/yt/…` YouTube references **and** it sends no CORS). The robustness goal ("if one fails / a song's missing") is met instead by the **merged multi-source search** above, not by these.
- `js/songs.js` — [Audius](https://audius.org) (no key, `app_name` only). **Full songs** via `/v1/tracks/trending` + `/v1/tracks/search`; `/v1/tracks/{id}/stream` returns `audio/mpeg`. CORS OK. Also powers the home "Trending Songs".
...
Unified item shape `{kind, src, id, name, sub, url, duration, art, vid?, preview?}`. YT items carry `vid` (no `url` until resolved); favourites persist `src`/`vid`/`preview` so YT/preview items replay.

**Search screen** has two zones tracked by `screen.sel` (0 = text input `#q`, 1 = the visible **▶ SEARCH** button). Relying on `change`/Enter alone was unreliable on the handset, so the flow is: type → **▼** moves focus to the SEARCH button (which **blurs the input, committing its value**) → **5/OK** runs `searchAll`. Number keys are never used to navigate the search screen (they must stay free for the device's multi-tap text entry); only the **D-pad** (ArrowUp/Down) moves between zones. Enter/OK directly from the field and LSK/Escape also submit, as conveniences.
...
- The `now` screen is a player UI (artwork from Audius/favicon, progress bar for tracks, ● LIVE for radio, circular transport buttons). **Volume is the phone's hardware keys, not the app** — there's no web API to set device volume, `audio.volume` is ignored on Cloud Phone, and the Web-Audio GainNode route would mute playback (Web-Audio output doesn't reach the handset). So on the now-screen Up/Down are deliberately **passed through** (no `preventDefault`, no handling) so the device's native volume can act; the app only offers Mute (`0` → `audio.muted`). Do NOT re-add an in-app volume bar — it's a no-op on the device. **YouTube is intentionally not used**: extracting audio needs a server + breaks YouTube ToS, and embedding is video/DRM-heavy — there's no static, legal, free way to stream mainstream songs (licensing), which is why Audius is the source.
